...
file_put_contents(__DIR__ . '/raw.txt', $raw);
...
file_put_contents(__DIR__ . '/decoded.txt', print_r($input, true));
...
// Validáció
if (!filter_var($data['email'], FILTER_VALIDATE_EMAIL) || empty($data['name'])) {
...
if (!$mail->send()) {
  file_put_contents(__DIR__ . '/mail-error.txt', $mail->ErrorInfo);
...
echo json_encode(['status' => 'success', 'message' => 'Email elküldve']);
